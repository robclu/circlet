use circlet::{channel, consumer, producer, UnpaddedRingbuffer};
...
fn unpadded_ringbuffer_simple_operations() {
    let buffer = Arc::new(UnpaddedRingbuffer::<u32>::new(11));
...
fn unpadded_try_push_and_pop() {
    let buffer = Arc::new(UnpaddedRingbuffer::<i32>::new(1));
    let (mut producer, mut consumer) = channel(buffer.clone());
...
fn unpadded_stress_test() {
    ...
    let buffer = Arc::new(UnpaddedRingbuffer::<usize>::new(threads_half as u32));
